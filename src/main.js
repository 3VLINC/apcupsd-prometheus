...
var socketPath = { host: "localhost", port: 3551 };
var outputFolder = resolve(__dirname, "./textfile_collector");
var outputFilename = process.env.APCUPSD_OUTPUT_FILEPATH;
var outputPath = resolve(outputFolder, outputFilename);
var refreshInterval = parseInt(process.env.APCUPSD_REFRESH_INTERVAL);
var debug = process.env.APCUPSD_DEBUG ? true : false;
...
const formatInternalTemperature = (text) => parseFloat(text.replace(' C Internal', ''));
const formatAmbientTemperature = (text) => parseFloat(text.replace(' C Ambient', ''));
...
var parseData = (data) => {
  ...
  const labelsOutput = 
    labels
    .filter(
      (item) => data[item.key] !== undefined
    ).map(
    (item) => 
  `${item.metric}="${trim(data[item.key])}"`).join(', ');
  ...
  const infoOutput = `# HELP apcupsd_info Information about UPS\n# TYPE apcupsd_info\napcupsd_info{${
      ...
      (item) => 
    `${item.metric}="${trim(data[item.key])}"`
  ).join(', ')}} 1.0\n`;
...
`);
  ...
  client
    .connect(socketPath.host, socketPath.port)
    .then(async () => {
      var update = async () => {
        lastUpdate = getTime();
        const data = await client.getStatusJson();

        await fs.writeFile(outputPath, parseData(data));

        var time = getTime();
        var timeUntilUpdate = lastUpdate + refreshInterval - time;
